...
char msg[50];
int value = 0;
const char* device_name = "ESP32(0)";  // <-- naam apparaat
...
int seagreen = CRGB::SeaGreen;
int purple = CRGB::Purple;


//colors
int kleuren[7] = { red, yellow, green, blue, black, seagreen, purple };
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "0") {
    ...
    if (messageTemp == "on") {
      client.publish("0", "Message recieved at ESP32(0)");
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("ESP32(0)")) {
      Serial.println("connected");
      // Subscribe
      client.subscribe("0");
      client.publish("0", "nr:0 online");
      for (int i = 0; i < ledaantal; i++) {
        leds[i] = kleuren[2];
      }
      FastLED.show();
...
void publishVoltage(int voltage) {
  ...
  client.publish("bat0", message.c_str());
